 ...
 */
package ua.edu.chmnu.ki.networks.tcp.mouse.client;

import ua.edu.chmnu.ki.networks.mouse.model.MouseCapture;
import ua.edu.chmnu.ki.networks.tcp.core.client.OneWayTcpClient;
...
public class MouseCaptureOperator {
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        try (ExecutorService executor = Executors.newSingleThreadExecutor()) {
            OneWayTcpClient<MouseCapture> oneWayTcpClient = new OneWayTcpClient<>(host, port, () -> {
                PointerInfo mousePointerInfo = MouseInfo.getPointerInfo();

                return new MouseCapture(mousePointerInfo.getLocation());
            });

            executor.submit(oneWayTcpClient);
